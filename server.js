...
// Из заголовков вычищаем ТОЛЬКО «ручку» 👋 (декор-приветствие) — с тоном кожи и VS16.
// Остальные эмодзи владелец сознательно оставляет в заголовках как есть.
const WAVE_RE = /\u{1F44B}[\u{1F3FB}-\u{1F3FF}\u{FE0F}]*/gu;

function firstLine(text, max = 110) {
  const lines = text.split('\n')
    .map(l => l.replace(WAVE_RE, '').replace(/\s+/g, ' ').trim())
    .filter(l => l.length > 0); // строка из одной «ручки» заголовком быть не может
...
<header>
  ...
  <div>
    ...
    <p class="social"><span class="sgroup"><span>Telegram:</span> ${config.columns.map(c => `<a href="https://t.me/${c.tg}" target="_blank" rel="noopener">${escapeHtml(c.title)}</a>`).join(' · ')}</span><span class="sep"> · </span><span class="sgroup"><span>YouTube:</span> ${config.columns.map(c => c.youtube
        ...
        : `<span>${escapeHtml(c.title)} скоро</span>`).join(' · ')}</span>${o.photos ? `<span class="sep"> · </span><span class="sgroup"><span>Мои фото на</span> <a href="${escapeHtml(o.photos)}" target="_blank" rel="noopener">wfolio.pro</a></span>` : ''}</p>
  </div>
